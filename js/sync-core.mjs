...
const recurrenceRule=value=>value&&typeof value==='object'&&!Array.isArray(value)?copy(value):null;
...
export const FIELD_GROUPS = Object.freeze({
  tasks: { title:['title'], date:['date'], time:['time'], duration:['duration'], category:['category'], priority:['priority'], challenge:['challenge'], recurrence:['recurrenceSeriesId','recurrenceRule','recurrenceOccurrence'], completion:['completed','completedAt'] },
  habits: { title:['title'] },
...
function versions(collection, row) {
  ...
  if (row?.syncVersions?.v === 1) {
    const meta=copy(row.syncVersions);
    meta.fields=obj(meta.fields);
    for(const key of Object.keys(FIELD_GROUPS[collection]))if(!meta.fields[key])meta.fields[key]=copy(meta.life?.stamp||stamp);
    return meta;
  }
...
export function collectionsFor(state) {
  ...
  for(const [collection,records] of Object.entries(result))for(const [id,row] of records) {
    ...
    if(meta?.v===1)row.syncVersions=versions(collection,{...row,syncVersions:meta});
  }
  return result;
}
export function prepareState(state, previous=null, now=Date.now(), actor=deviceActor) {
  ...
  for(const [collection,records] of Object.entries(result))for(const [id,row] of records) {
    ...
    const meta=versions(collection,{...row,syncVersions:oldMeta[key] || old?.syncVersions});
...
function stateFromCollections(template,collections){
  ...
  next.tasks=active('tasks').map(row=>({id:row.id,title:row.title,date:row.date||'',time:row.time||'',duration:row.duration??null,category:row.category,priority:row.priority,challenge:row.challenge,recurrenceSeriesId:row.recurrenceSeriesId||null,recurrenceRule:recurrenceRule(row.recurrenceRule),recurrenceOccurrence:row.recurrenceOccurrence||null,completed:row.completed,completedAt:row.completedAt?asLocal(row.completedAt):null,createdAt:asLocal(row.createdAt),updatedAt:asLocal(row.updatedAt)}));
  ...
  for(const [collection,records] of Object.entries(collections))for(const [id,row] of records)if(row.syncVersions)next.syncMeta.fieldVersions[collection+':'+id]=copy(versions(collection,row));
