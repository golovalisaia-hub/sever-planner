export const SEVER_MANIFEST = {
  version:2,
  pages:{today:'Сегодня: задачи дня',calendar:'Календарь',notes:'Заметки и папки',timer:'Таймер фокуса',progress:'Прогресс',habits:'Привычки',money:'Деньги: долги и накопления',settings:'Настройки'},
...
export function toolsFor(page:string,role:'owner'|'user',message='') {
  ...
  if(page==='money'||/план|цель|накоп|долг|чита|экзамен/i.test(message))tools.push('plan.create','plan.get');
