...
const RELEASE_CACHE = 'sever-v106-missed-tasks-release-v1';

test('current service worker installs the guarded v106 release assets atomically and removes stale caches', async () => {
...
test('installed current release serves v106 planner core and v104 Notes responsiveness from one release cache', async () => {
  const handlers = new Map();
  const requests = [];
  let network = 0;
  const self = {
  ...
  const caches = {
    open: async name => {
      ...
      return { match: async key => { requests.push(key); return { release: 106, key }; } };
    }
  };
