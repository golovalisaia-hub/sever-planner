...
test('installed current release serves v99 experience and core planner assets from one release cache', async () => {
  const handlers = new Map();
  const requests = [];
  let network = 0;
  const self = {
  ...
  const caches = {
    open: async name => {
      ...
      return { match: async key => { requests.push(key); return { release: 99, key }; } };
    }
  };
  ...
  const cases = [
    ...
    ['sever2-experience-v94.js?v=old','cors','./sever2-experience-v94.js?v=99'],
