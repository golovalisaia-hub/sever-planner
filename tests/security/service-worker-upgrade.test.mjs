...
const RELEASE_CACHE = 'sever-v101-reliability-release-v1';

test('current service worker installs the guarded v101 release atomically and removes stale caches', async () => {
...
test('installed current release serves v101 reliability and core planner assets from one release cache', async () => {
  const handlers = new Map();
  const requests = [];
  let network = 0;
  const self = {
  ...
  const caches = {
    open: async name => {
      ...
      return { match: async key => { requests.push(key); return { release: 101, key }; } };
    }
  };
