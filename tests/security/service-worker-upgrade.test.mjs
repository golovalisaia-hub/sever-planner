...
assert.ok(RELEASE_CACHE.startsWith('sever-v107-'), 'security suite must run against the v107 atomic release');

test('current service worker installs the guarded v107 release assets atomically and removes stale caches', async () => {
  ...
  const caches = {
    ...
    keys: async () => [
      ...
      'sever-v102-desktop-polish-release-v1',
...
test('installed current release serves v106 planner core, v104 Notes and v107 interaction polish from one release cache', async () => {
  const handlers = new Map();
  const requests = [];
  let network = 0;
  const self = {
  ...
  const caches = {
    open: async name => {
      ...
      return { match: async key => { requests.push(key); return { release: 107, key }; } };
    }
  };
  ...
  const cases = [
    ...
    ['sever2-interaction-polish.js?v=old','cors','./sever2-interaction-polish.js?v=107'],
