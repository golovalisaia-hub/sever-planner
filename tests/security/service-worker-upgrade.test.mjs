...
const RELEASE_CACHE = 'sever-v94-experience-release-v1';
...
test('current service worker installs the guarded release atomically and removes stale caches', async () => {
  ...
  const caches = {
    ...
    keys: async () => ['sever-v82-reminders-desktop-v9', 'sever-v92-unified-release-v1', 'sever-v93-complete-release-v1'],
  ...
  await activateWork;
  assert.ok(deleted.includes('sever-v82-reminders-desktop-v9'));
  assert.ok(deleted.includes('sever-v92-unified-release-v1'));
  assert.ok(deleted.includes('sever-v93-complete-release-v1'));
  assert.ok(!deleted.includes(RELEASE_CACHE));
  assert.equal(claimed, true);
  assert.equal(skipped, true);
});

test('installed current release serves v94 experience and core planner assets from one release cache', async () => {
  const handlers = new Map();
  const requests = [];
  let network = 0;
  const self = {
  ...
  const caches = {
    open: async name => {
      ...
      return { match: async key => { requests.push(key); return { release: 94, key }; } };
    }
  };
  ...
  const cases = [
    ...
    ['sever2-experience-v94.css?v=old','cors','./sever2-experience-v94.css?v=94'],
    ['sever2-experience-v94.js?v=old','cors','./sever2-experience-v94.js?v=94'],
