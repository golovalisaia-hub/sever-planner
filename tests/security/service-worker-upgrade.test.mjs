...
assert.ok(RELEASE_CACHE.startsWith('sever-v108-'), 'security suite must run against the v108 atomic release');

test('current service worker installs the guarded v108 release assets atomically and removes stale caches', async () => {
...
test('installed current release serves v106 planner core, v104 Notes and v108 interaction polish from one release cache', async () => {
  const handlers = new Map();
  const requests = [];
  let network = 0;
  const self = {
  ...
  const caches = {
    open: async name => {
      ...
      return { match: async key => { requests.push(key); return { release: 108, key }; } };
    }
  };
  ...
  const cases = [
    ...
    ['sever2-interaction-polish.js?v=old','cors','./sever2-interaction-polish.js?v=108'],
