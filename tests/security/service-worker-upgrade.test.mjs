...
const RELEASE_CACHE = 'sever-v55-field-sync-theme3-v1';

test('v55 service worker installs the three-theme release atomically and removes stale caches', async () => {
  ...
  const caches = {
    ...
    keys: async () => ['sever-v35', 'sever-v36-security', 'sever-v37-auth', 'sever-v38-brand', 'sever-v39-northern', 'sever-v40-northern', 'sever-v41-global-rebuild', 'sever-v41-global-rebuild-r2', 'sever-v42-sync-calendar-r1', 'sever-v43-sync-audit-r1', 'sever-v55-field-sync'],
...
test('installed release serves HTML and critical assets from the same three-theme cache even online', async () => {
  ...
  const caches = { open: async name => { assert.equal(name, RELEASE_CACHE); return { match: async key => { requests.push(key); return { release: 55, key }; } }; } };
  vm.runInNewContext(source, { self, caches, URL, Response, fetch: async () => { network++; throw Error('network must not update a release'); } });
  for (const [path, mode, expected] of [['?verify=new','navigate','./index.html'],['mobile-home.css?v=old','cors','./mobile-home.css?v=52'],['app.js?v=new','cors','./app.js?v=51'],['themes.css?v=old','cors','./themes.css?v=57']]) {
    let response;
