...
const RELEASE_CACHE = 'sever-v66-home-flow-v1';

test('v66 service worker installs the Home flow release atomically and removes stale caches', async () => {
  ...
  assert.ok(cachedAssets.includes('./sever2-home-flow.css?v=70'));
  assert.ok(cachedAssets.includes('./sever2-home-flow.js?v=70'));
...
test('installed release serves HTML and critical Home assets from one release cache', async () => {
  ...
  for (const [pathValue, mode, expected] of [
    ...
    ['sever2-home-flow.css?v=old','cors','./sever2-home-flow.css?v=70'],
    ['sever2-home-flow.js?v=old','cors','./sever2-home-flow.js?v=70'],
