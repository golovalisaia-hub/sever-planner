...
test('v46 service worker installs AI assets atomically and removes stale caches', async () => {
  ...
  const caches = {
    open: async name => ({ addAll: async assets => { assert.equal(name, 'sever-v46-cloud-diagnostics'); cachedAssets = assets; }, put: async () => {} }),
