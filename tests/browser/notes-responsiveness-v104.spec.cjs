...
test('checklist taps update the existing card immediately and survive reload without a full Notes rebuild', async ({ page }) => {
  ...
  expect(await page.evaluate(() => {
    const stored = JSON.parse(localStorage.getItem('sever-anonymous-state-v1'));
    const note = stored.notes.find(item => item.id === 'note-v104');
    return Boolean(note?.done && note?.items?.every(item => item.done));
  })).toBe(true);
  ...
  await page.evaluate(() => window.SeverApp.switchView('notes'));
  await expect.poll(() => page.evaluate(() => document.documentElement.dataset.severNotesResponsiveness)).toBe('v104');
  const reloadedCard = page.locator('.note-card[data-note-id="note-v104"]');
  await expect(reloadedCard).toHaveClass(/complete/);
  await expect(reloadedCard.locator('.note-ring b')).toHaveText('100%');
  const reloadedChecks = reloadedCard.locator('.note-checklist input[type="checkbox"]');
  await expect(reloadedChecks).toHaveCount(4);
  for (let index = 0; index < 4; index += 1) await expect(reloadedChecks.nth(index)).toBeChecked();
});
