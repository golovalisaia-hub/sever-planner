...
test('installed release reloads offline with one complete asset set', async ({ browser }) => {
  ...
  try {
    ...
    const cached = await page.evaluate(async () => { const cache = await caches.open('sever-v57-unified-sever2-v2'); return (await cache.keys()).map(request => new URL(request.url).pathname + new URL(request.url).search); });
    ...
    expect(cached).toContain('/js/theme-init.js?v=61');
