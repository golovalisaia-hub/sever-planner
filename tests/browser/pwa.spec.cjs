...
test('installed release reloads offline with one complete asset set', async ({ browser }) => {
  ...
  try {
    ...
    const cached = await page.evaluate(async () => { const cache = await caches.open('sever-v55-field-sync-theme3-v1'); return (await cache.keys()).map(request => new URL(request.url).pathname + new URL(request.url).search); });
    ...
    expect(cached).toContain('/js/sync-core.mjs?v=55'); expect(cached).toContain('/js/cloud-runtime.js?v=55'); expect(cached).toContain('/themes.css?v=57');
