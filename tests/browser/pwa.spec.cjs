    ...
    await expect.poll(async () => {
      try {
        cached = await page.evaluate(async () => {
          const cache = await caches.open('sever-v73-money-v1');
          return (await cache.keys()).map(request => new URL(request.url).pathname + new URL(request.url).search);
        });
        return cached.includes('/sever2-notes-core.js?v=71') && cached.includes('/sever2-notes-organization.js?v=72') && cached.includes('/sever2-notes-editor-flow.js?v=73') && cached.includes('/sever2-notes-navigation.js?v=74') && cached.includes('/sever2-notes-polish.js?v=75') && cached.includes('/sever2-mobile-consistency.css?v=76') && cached.includes('/sever2-money.js?v=77') && cached.includes('/js/theme-init.js?v=77');
      } catch { return false; }
    }).toBe(true);
    for (const asset of [
      ...
      '/sever2-mobile-consistency.css?v=76','/sever2-money.css?v=77','/sever2-money.js?v=77','/js/theme-init.js?v=77','/app.js?v=51','/notes-pro.js?v=52','/js/sync-core.mjs?v=55','/js/cloud-runtime.js?v=55'
    ]) expect(cached).toContain(asset);
    expect(cached).not.toContain('/desktop-home.css?v=60');
    await context.setOffline(true);
    await page.reload();
    await expect(page.locator('#todayView')).toBeVisible();
    for (const key of ['severProductivity','severFocusFlow','severEfficiency','severCalendarClarity','severCreateFlow','severHomeCore','severNotesCore','severNotesOrganization','severNotesEditorFlow','severNotesNavigation','severNotesPolish','severMoney']) {
