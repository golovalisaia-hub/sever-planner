...
test('installed release reloads offline with one complete asset set', async ({ browser }) => {
  ...
  try {
    ...
    await expect.poll(async () => {
      try {
        cached = await page.evaluate(async () => {
          const cache = await caches.open('sever-v66-home-flow-v1');
          return (await cache.keys()).map(request => new URL(request.url).pathname + new URL(request.url).search);
        });
        return cached.includes('/sever2-home-flow.js?v=70') && cached.includes('/js/theme-init.js?v=70');
      } catch {
        return false;
      }
    }).toBe(true);
    ...
    expect(cached).toContain('/sever2-home-flow.css?v=70');
    expect(cached).toContain('/sever2-home-flow.js?v=70');
    expect(cached).toContain('/js/theme-init.js?v=70');
    ...
    await expect.poll(() => page.evaluate(() => document.documentElement.dataset.severHomeFlow)).toBe('ready');
    await expect(page.locator('#sever2HomeCommand')).toBeVisible();
