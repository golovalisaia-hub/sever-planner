...
test('installed release reloads offline with one complete asset set', async ({ browser }) => {
  ...
  try {
    ...
    const cached = await page.evaluate(async () => { const cache = await caches.open('sever-v53-multi-theme'); return (await cache.keys()).map(request => new URL(request.url).pathname + new URL(request.url).search); });
    expect(cached).toContain('/mobile-home.css?v=52'); expect(cached).toContain('/app.js?v=53'); expect(cached).toContain('/notes-pro.js?v=52');
