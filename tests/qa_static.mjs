...
const sql001 = read('supabase/migrations/001_initial_cloud_sync.sql');
const sql002 = read('supabase/migrations/002_harden_tenant_relations.sql');
...
test('Cloud sync has a safe static UI and public-only configuration', () => { assert.ok(fs.existsSync(path.join(root, 'supabase-config.js'))); assert.ok(fs.existsSync(path.join(root, 'js', 'cloud-runtime.js'))); assert.ok(fs.existsSync(path.join(root, 'supabase', 'migrations', '001_initial_cloud_sync.sql'))); assert.ok(fs.existsSync(path.join(root, 'supabase', 'migrations', '002_harden_tenant_relations.sql'))); assert.match(html, /id="accountDialog"/); assert.match(html, /id="migrationDialog"/); assert.match(html, /js\/cloud-runtime\.js\?v=25/); assert.match(read('supabase-config.js'), /anonKey:\s*'sb_publishable_/); assert.doesNotMatch(read('supabase-config.js'), /service_role[_\\s]*key\\s*:/i); });
test('Local-only choice truly disables cloud queue, pull, retry and realtime', () => { assert.match(cloud, /get localOnly\(\)/); assert.match(cloud, /if \(!this\.user \|\| this\.localOnly \|\| !operations\.length\) return/); assert.match(cloud, /write\(this\.queueKey, \[\]\)/); assert.match(cloud, /if \(!this\.user \|\| this\.localOnly \|\| !navigator\.onLine \|\| this\.running\) return/); assert.match(cloud, /if \(this\.subscription \|\| !this\.user \|\| this\.localOnly\) return/); });
test('Logout restores the saved anonymous state instead of replacing it with empty data', () => { assert.match(cloud, /ANONYMOUS_STATE_KEY/); assert.match(cloud, /const anonymousState = read\(ANONYMOUS_STATE_KEY, this\.app\.freshState\(\)\)/); assert.match(cloud, /switchStorageScope\(null, anonymousState\)/); });
test('Legacy data ownership is assigned only after accepted cloud migration', () => { const writes = [...cloud.matchAll(/localStorage\.setItem\(LEGACY_OWNER_KEY/g)]; assert.equal(writes.length, 1); assert.match(cloud, /async acceptMigration\(\)[\s\S]*localStorage\.setItem\(LEGACY_OWNER_KEY, this\.user\.id\)/); });
...
test('Supabase RLS migrations are rerunnable and owner-aware', () => { assert.match(sql001, /drop policy if exists "tasks are private"/); assert.match(sql001, /drop policy if exists "notes are private"/); assert.match(sql002, /foreign key \(user_id, habit_id\)/); assert.match(sql002, /foreign key \(user_id, folder_id\)/); assert.match(sql002, /foreign key \(user_id, task_id\)/); });
...
test('Focus sessions are primary cloud data and timer does not write every tick', () => { assert.match(app, /focusSessions:\[\]/); assert.match(app, /state\.focusSessions\.push/); assert.match(app, /if\(now-lastFocusPersistAt>=15000\)/); assert.match(sync, /export function focusStatsFor/); assert.match(sync, /next\.stats=focusStatsFor\(next\.focusSessions\)/); });
