...
test('Asset versions are aligned to v30', () => { for (const asset of ['style.css', 'qa.css', 'responsive.css', 'design-system.css', 'app.js', 'notes-pro.js']) { assert.match(html, new RegExp(`${asset.replace('.', '\\.')}\\?v=30`)); assert.match(sw, new RegExp(`${asset.replace('.', '\\.')}\\?v=30`)); } assert.match(sw, /sever-v30/); });
...
test('Dashboard and quick capture use real planner state', () => { assert.match(html, /id="todayDashboard"/); assert.match(html, /id="dashboardNextStart"/); assert.match(html, /id="quickCaptureForm"/); assert.match(app, /function renderDashboard/); assert.match(app, /focusMinutesForDate/); assert.match(app, /todayFilter='all'/); assert.match(read('notes-pro.js'), /noteSearchQuery/); });
test('Desktop habit summary exposes a real Monday–Sunday completion trail', () => { assert.match(app, /weekLabels=\['Пн','Вт','Ср','Чт','Пт','Сб','Вс'\]/); assert.match(app, /habit-week-markers/); assert.match(css, /\.habit-week-markers/); });
