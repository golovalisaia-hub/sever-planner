(() => {
  ...
  function todayTasks() {
    const today = TODAY();
    return state().tasks.filter(task => !task.challenge && task.date === today);
  }

  function inboxTasks() {
    return state().tasks.filter(task => !task.challenge && !task.completed && (!task.date || task.date === INBOX_DATE));
  }
  ...
  function ensureShell() {
    ...
    shell.innerHTML = `
      <div class="sever2-home-focus-head">
        <div>
          <small>СЕГОДНЯ</small>
          <h2>Главное на день</h2>
          <p id="sever2HomeFocusSummary">План на сегодня</p>
        </div>
        <button id="sever2HomeCreate" type="button">+ Создать</button>
      </div>
  ...
  function render() {
    ...
    const top = rankTasks(pending).slice(0, 3);

    $('#sever2HomeFocusSummary').textContent = all.length
      ? `${pending.length} осталось · ${done.length} готово${minutes ? ` · ${minutes} мин` : ''}`
      : 'День свободен — добавьте только то, что действительно нужно';
    $('#sever2HomeInboxCount').textContent = inbox.length
      ? `${inbox.length} ${inbox.length === 1 ? 'задача' : 'задач'} без даты`
      : 'Нет задач без даты';

    const list = $('#sever2HomeTopTasks');
    list.replaceChildren();
    if (!top.length) {
      const empty = document.createElement('div');
      empty.className = 'sever2-home-focus-empty';
      ...
      top.forEach((task, index) => {
        ...
        row.className = 'sever2-home-focus-task';
        row.dataset.taskId = task.id;
        const meta = [task.time, task.duration ? `${task.duration} мин` : '', task.category || ''].filter(Boolean).join(' · ');
        row.innerHTML = `
          <span class="sever2-home-focus-number">${index + 1}</span>
          <button class="sever2-home-focus-copy" type="button"><b></b><small></small></button>
  ...
  function boot() {
    ...
    if (tasks && !observer) {
      ...
      observer.observe(tasks, { childList: true, subtree: true, attributes: true, attributeFilter: ['class'] });
    }
  ...
  const tryBoot = () => {
    if (boot()) return;
    if (attempts++ < 120) setTimeout(tryBoot, 50);
  };
